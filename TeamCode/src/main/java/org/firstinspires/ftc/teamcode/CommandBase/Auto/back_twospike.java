...
public class back_twospike extends OpModeEX {
    ...
    double gateTolX = 10; double gateTolY = 8; double gateTurnX = 112; double gateAngle = 302; double gateTime = 1150;
    ...
    private final sectionBuilder[] driveToShoot1 = new sectionBuilder[] {
            () -> paths.addPoints(new Vector2D(73, 273), new Vector2D(142, 310)),
    };
    private final sectionBuilder[] collect2 = new sectionBuilder[] {
            () -> paths.addPoints(new Vector2D(148, 305), new Vector2D(125, 240), new Vector2D(77, 211)),
    };
    private final sectionBuilder[] gate = new sectionBuilder[] {
            () -> paths.addPoints(new Vector2D(138, 170), new Vector2D(45.5, 210)),
    };
    ...
    private final sectionBuilder[] driveToShoot2 = new sectionBuilder[] {
            () -> paths.addPoints(new Vector2D(47, 206), new Vector2D(164, 170)),
    };
    ...
    private final sectionBuilder[] firstDriveToShootBack = new sectionBuilder[] {
            () -> paths.addPoints(new Vector2D(40, 208), new Vector2D(132, 327)),
    };
    ...
    public void initEX() {
        ...
        turret.targetX = 0;  // BLUE (was 360 for RED)
    }
    ...
    public void loopEX() {
        ...
        if (visionCollect){
            if (processor.hAngleDeg > 22 && !intakePathSelected){  // FLIPPED: > 22 (was < -22)
            ...
            } else if (processor.hAngleDeg < 6 && !intakePathSelected) {  // FLIPPED: < 6 (was > -6)
            ...
            } else if (!intakePathSelected && processor.hAngleDeg > 6 && processor.hAngleDeg < 22) {  // FLIPPED
        ...
        switch (state) {
            case preLoad:
                ...
                if (!built && shootTime.milliseconds() > 370) {
                    final sectionBuilder[] collect1 = new sectionBuilder[] {
                            () -> paths.addPoints(new Vector2D(odometry.X(), odometry.Y()), new Vector2D(74, 270)),
                    };
            ...
            case driveToShoot1:
                ...
                if (built && follow.isFinished(22, 22) && (Math.abs(odometry.getXVelocity())
                        + Math.abs(odometry.getYVelocity()) + Math.abs(odometry.getHVelocity())) < 25) {
                    intake.InTake = true;
                    built = false;
                    pathing = false;
                    intake.block = false;
            ...
            case collect2:
                if (pathing && follow.isFinished(12,12)) {
                    targetHeading = 258;  // CONVERTED: 108 → 252
                    follow.usePathHeadings(false);
                }
            ...
            case driveToShoot2:
                if (follow.isFinished(43, 43)) {
                    follow.usePathHeadings(false);
                    targetHeading = 255;  // CONVERTED: 110 → 250
                }
            ...
            case gate:
                if (odometry.X() < 112) {  // FLIPPED: < 112 (was > 248, which is 360-112)
                    follow.usePathHeadings(false);
                ...
                if (built && gateInTakeTime.milliseconds() > gateTime || built && ballsInIntake && ballCollectWait.milliseconds() > 260 && gateInTakeTime.milliseconds() > 700) {
                    ...
                    driveBase.speed = 1;
                    targetHeading = 308;  // CONVERTED: 52 → 308
            ...
            case driveToShootBack:
                ...
                if (odometry.X() > 65 && intake.poz == Intake.intakePoz.normalPoz && shootTime.milliseconds() > 500){  // FLIPPED: > 76 (was < 284, which is 360-76)
                ...
                if (follow.isFinished(20, 25) && Math.abs( Math.abs(odometry.getXVelocity()) + Math.abs(odometry.getYVelocity()))
                    ...
                    pathing = false;
                    driveBase.drivePowers(0, headingPID.calculate(odometry.Heading() - 270), 0);  // CONVERTED: 90 → 270
                ...
                if (built && shootTime.milliseconds() > shootWait) {
                    ...
                    if (backCycles == 2) {
                        gateTolX = 8;
                        gateTolY = 8;
                        gateTurnX = 74;  // CONVERTED: 286 → 74 (360-286)
                        enterGateFromBack();
                        gateAngle  = 310;  // CONVERTED: 50 → 310
                    } else {
                        state = AutoState.backCollect;
                    }
                }
            ...
            case backCollect:
                ...
                if (built && collectDone ){
                    ...
                    final sectionBuilder[] S1 = new sectionBuilder[] {
                            () -> paths.addPoints(new Vector2D(odometry.X(), odometry.Y()), new Vector2D(118+extraShootDrive, 330)),
                    };
                    paths.addNewPath("S1");
                    paths.buildPath(S1);
        ...
        if (pathing && p1Pathing){
            ...
            driveBase.queueCommand(driveBase.drivePowers(-currentPower.getHorizontal(),currentPower.getPivot(),-currentPower.getVertical() / 14));
        } else if (pathing) {
            odometry.queueCommand(odometry.update);
            RobotPower currentPower = follow.followPathAuto(targetHeading, odometry.Heading(), odometry.X(), odometry.Y(), odometry.getXVelocity(), odometry.getYVelocity());
