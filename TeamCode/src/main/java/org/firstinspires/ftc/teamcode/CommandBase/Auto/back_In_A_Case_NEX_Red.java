...
@Autonomous
public class back_In_A_Case_NEX_Red extends OpModeEX {
    ...
    PIDController headingPID = new PIDController(0.010,0,0.0030);
    ...
    private final sectionBuilder[] shoot = new sectionBuilder[]{
            () -> paths.addPoints(new Vector2D(196, 330), new Vector2D(196, 305)),
    };

    private final sectionBuilder[] collect1 = new sectionBuilder[]{
            () -> paths.addPoints(new Vector2D(211, 330), new Vector2D(240, 254), new Vector2D(308, 268)),
    };

    private final sectionBuilder[] driveToShoot1 = new sectionBuilder[]{
            () -> paths.addPoints(new Vector2D(323, 270), new Vector2D(262, 261), new Vector2D(236, 300)),
    };

    private final sectionBuilder[] collect2 = new sectionBuilder[]{
            () -> paths.addPoints(new Vector2D(221, 310), new Vector2D(234, 190), new Vector2D(311, 220)),
    };

    private final sectionBuilder[] gate = new sectionBuilder[]{
            () -> paths.addPoints(new Vector2D(281, 223), new Vector2D(261, 215), new Vector2D(311, 170)),
    };

    private final sectionBuilder[] driveToShoot2 = new sectionBuilder[]{
            () -> paths.addPoints(new Vector2D(331, 210), new Vector2D(226, 165)),
    };

    private final sectionBuilder[] collect3 = new sectionBuilder[]{
            () -> paths.addPoints(new Vector2D(226, 155), new Vector2D(304, 148)),
    };

    private final sectionBuilder[] driveToShoot3 = new sectionBuilder[]{
            () -> paths.addPoints(new Vector2D(326, 150), new Vector2D(212, 130)),
    };

    private final sectionBuilder[] firstBackCollect = new sectionBuilder[]{
            () -> paths.addPoints(new Vector2D(226, 150), new Vector2D(256, 300)),
    };

    private final sectionBuilder[] driveToShootBack = new sectionBuilder[]{
            () -> paths.addPoints(new Vector2D(314, 329), new Vector2D(224, 329)),
    };

    @Override
    public void initEX() {
        odometry.startPosition(197, 346, 190);
        turret.Auto = true;
        ...
        builder.setCamera(hardwareMap.get(WebcamName.class, "Webcam 1")); // Set lower resolution here
        builder.setCameraResolution(new Size(640, 480)); // or 640x480 if you want
        builder.setStreamFormat(VisionPortal.StreamFormat.MJPEG); // Optional: disable RC live view to save CPU
    ...
    public void loopEX() {
        ...
        if (visionCollect){
            driveBase.drivePowers( 0.2 + Math.pow(processor.distanceCm / 65,2), headingPID.calculate(processor.hAngleDeg), 0);
            intake.block = true;
            intake.InTake = true;
        }
        switch (state) {
