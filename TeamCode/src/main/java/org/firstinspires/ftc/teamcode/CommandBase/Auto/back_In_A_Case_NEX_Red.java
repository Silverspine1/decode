...
public class back_In_A_Case_NEX_Red extends OpModeEX {
    ...
    private final sectionBuilder[] driveToShoot1 = new sectionBuilder[]{
            () -> paths.addPoints(new Vector2D(317, 270), new Vector2D(256, 261), new Vector2D(230, 320)),
    };
    ...
    public void loopEX() {
        ...
        if (visionCollect) {
            driveBase.drivePowers(0.2 + Math.pow(processor.distanceCm / 65, 2), headingPID.calculate(processor.hAngleDeg), 0); // No negation for Red
            intake.block = true;
            intake.InTake = true;
        }
        ...
        switch (state) {
            ...
            case backCollect:
                if (built && maxWait.milliseconds() < 220 || intake.ballCount > 2) {
                    visionCollect = true;
                }
                if (built && maxWait.milliseconds() > 2200 || intake.ballCount > 2) {
                    visionCollect = false;
