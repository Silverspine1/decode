    ...
    public void loopEX() {
        ...
        switch (state) {
            ...
            case driveToShoot1:
                ...
                if (built && follow.isFinished(15,15) && (Math.abs(odometry.getXVelocity())+ Math.abs(odometry.getYVelocity()) + Math.abs(odometry.getHVelocity()))< velo/1.6 && extraTurnWait.milliseconds() > 400){
                    intake.InTake = true;
                    built = false;
                    intake.block = false;
                    shootTime.reset();
